...
def merge_concat_df(df_srx_srr_drop, df_missed_recorver):
    '''Receives two dfs. The second df (missed recover) will be
    merged with the first one to get all desired columns.
    After, both dfs will be concatenated. The function will
...
def split_SRR(df_drop_na):
    '''This function receives a df where some samples (GSM) have
    more then one SRR. This function will split the SRR list and 
...
def srr_gsm_metadata(srr_split_final, df_drop_na):
    '''Receives the splited srr_gsm file. Returns
    a df with the SRR per GSM (no splited) and the
    SRR count per GSM'''
